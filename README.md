...
An org with hosts of very different real capacity needs more than one **scale-set profile**, not one release tuned down to the smallest node: a single runner pod template's resource limits apply verbatim regardless of which node the scheduler actually places a pod on, so one release sized for a small node would silently cap every job org-wide, while one sized for a large node risks OOMing the small one outright if the scheduler ever places a pod there. `github_runner_arc_orgs[].scale_set_profiles` is a list of `{suffix, values_file, node_selector}` entries - one Helm release, one namespace, and one `nodeSelector` per entry, all pooled under the same `runs-on` label via `scaleSetLabels` (not `runnerScaleSetName`, which is a scale set's own unique GitHub-side registration identity, not just a label - two releases sharing it collide outright). See `ansible/host_vars/node-a.yml` for ExaDev's own two-profile setup (node-b's original values pinned to itself, node-a's own smaller-tuned values pinned to itself).
