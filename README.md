...
An org with hosts of very different real capacity needs more than one **scale-set profile**, not one release tuned down to the smallest node: a single runner pod template's resource limits apply verbatim regardless of which node the scheduler actually places a pod on, so one release sized for a small node would silently cap every job org-wide, while one sized for a large node risks OOMing the small one outright if the scheduler ever places a pod there. `github_runner_arc_orgs[].scale_set_profiles` is a list of `{suffix, values_file, node_selector, runs_on_label?}` entries - one Helm release, one namespace, and one `nodeSelector` per entry, pooled under the org's own shared `runs-on` label via `scaleSetLabels` (not `runnerScaleSetName`, which is a scale set's own unique GitHub-side registration identity, not just a label - two releases sharing it collide outright) unless a profile sets its own `runs_on_label` to deliberately opt out of that pool - e.g. a dedicated image-building profile ordinary CI jobs have no business landing on. See `ansible/host_vars/node-a.yml` for ExaDev's own three-profile setup: node-b's original values pinned to itself, node-a's own smaller-tuned values pinned to itself, and a third, separately-labelled, Docker-in-Docker profile for building the fleet's own runner image.
...
`values/exadev-runners-values.yaml`'s `maxRunners: 2` is the mathematically safe ceiling for any single job, proven against a real out-of-memory and pod-eviction incident (see that file's own comments). It leaves real idle capacity unused whenever the actual jobs in flight need less than that. The `autoscaler` service raises and lowers `maxRunners` on the single `exadev-runners` `AutoscalingRunnerSet` based on real, current memory usage and host pressure, never on job identity or GitHub Actions queue depth: every consuming repo's CI keeps using the single `exadev-runners` label unchanged, and no job is ever classified as light or heavy.
...
- Reads host/VM memory and swap pressure from `/proc/meminfo` inside the container. **Not** macOS's `vm_stat`/`sysctl vm.swapusage`: those are macOS-native and cannot run inside any Linux container, even a host-networked one — a container only ever shares a network namespace with the host that way, never its process or binary environment. `/proc/meminfo` inside an unprivileged, non-cgroup-virtualized container reflects the Colima VM's own real memory instead, which is the actual resource-constrained boundary the values file's 23.4Gi allocatable figure describes, and the actual mechanism behind the eviction/OOM incidents that figure was tuned against — arguably a more directly relevant signal than the raw macOS host would even be. This needs confirming empirically on the first real dry run, same as the `kubectl top pod` (metrics-server) assumption above.
- Raises `maxRunners` by exactly +1 per cycle (never straight to a computed target) once headroom has covered a full pod's hard limit for `AUTOSCALER_RAISE_CONFIRM_POLLS` (default 2) consecutive polls, capped at `AUTOSCALER_MAX_CEILING` (default 4, the original pre-incident value, as an operator belt-and-braces cap independent of the live arithmetic).
...
- Fails safe on any measurement error (`kubectl` unreachable, `/proc/meminfo` unreadable): patches down to `AUTOSCALER_FLOOR` (default 2) immediately rather than skipping the cycle silently.
