...
It sets `github_runner_cluster_k3s_token`, `github_runner_cluster_tailscale_join_key`, `github_runner_cluster_tailscale_api_token`, `github_runner_cluster_create_tailscale_join_key`, `github_runner_cluster_headscale_api_key`, `github_runner_cluster_headscale_join_key` and `github_runner_cluster_create_headscale_join_key` for the cluster role, and `github_runner_arc_ghcr_username`, `github_runner_arc_ghcr_token`, `github_runner_arc_heartbeat_gh_token` and `github_runner_arc_org_private_keys` for the ARC role. Either role works without this one if those variables are set some other way.
...
- `github_runner_secrets_provided`: the alternative to 1Password, a dict with the same keys (`k3s_token`, `ghcr_username`, `ghcr_token`, `heartbeat_gh_token`, `tailscale_join_key`, `tailscale_api_token`, `headscale_api_key`, `headscale_join_key`, `org_private_keys`, the Headscale ones optional). Non-empty means 1Password is not used at all.
- `github_runner_secrets_cache_path`: where the 1Password read is cached on the control node.

`tasks/persist_tailscale_join_key.yml` and `tasks/persist_headscale_join_key.yml` write a newly created Tailscale or Headscale join key back to the item and the cache; the cluster role includes them when it creates one.
